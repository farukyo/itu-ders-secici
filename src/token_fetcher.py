...
class ContinuousTokenFetcher(threading.Thread):
    ...
    def __init__(
        self, url: str, login: str, password: str, use_headless_browser: bool = False
    ) -> None:
    ...
    def login_to_kepler(self) -> None:
        ...
        if self.driver is None:
            self.driver = DriverManager.create_driver(
                headless=self.use_headless_browser
            )
        ...
        if "girisv3.itu.edu.tr" not in self.driver.current_url:
            Logger.log(
                "Kepler'e giriş yapılmış, giriş yapma aşaması atlanıyor...",
                silent=is_repeat,
            )
            return
        ...
        if "SelectIdentity" in self.driver.current_url:
            sleep(0.1)
            Logger.log(
                "Yatay geçiş hesabı algılandı, aktif İTÜ hesabı seçilecek.",
                silent=is_repeat,
            )
    ...
    def _fetch_token_once(self) -> str:
        ...
        if self.url not in self.driver.current_url:
            Logger.log(
                "Ders seçim sitesi açılıyor...", silent=self._started_event.is_set()
            )
            self.driver.get(self.url)
            sleep(PAGE_LOAD_DELAY)
        ...
        if "girisv3.itu.edu.tr" in self.driver.current_url:
            Logger.log(
                "Kepler hesabından çıkıldığı algılandı, tekrar giriş yapılıyor..."
            )
    ...
    def run(self) -> None:
        ...
        self._running = True
        Logger.log(
            "Token fetcher thread başlatıldı, sürekli token alınacak...", silent=True
        )
