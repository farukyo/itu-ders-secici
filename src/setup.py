...
ITU_HELPER_COURSES_URL = (
    "https://raw.githubusercontent.com/itu-helper/data/main/courses.psv"
)
...
def ask_for_crn_list(allow_backup_crns: bool) -> tuple[list[str], float, list[float]]:
    if allow_backup_crns:
        print(
            '   İPUCU: Yedek CRN girmek için "CRN:YEDEK_CRN" formatını kullanabilirsiniz.'
        )
        print('   Örnek: "21345:21346" → Asıl CRN: 21345, Yedek CRN: 21346')
        print(
            "   Yedek CRN, asıl CRN'nin kontenjanı dolduğunda otomatik olarak denenecektir.\n"
        )
    ...
    while last_inp != "":
        ...
        if primary_crn not in crn_to_lesson.keys():
            ...
            try:
                ...
                print(
                    f"Dersin ITU Helper veritabanında bulunan adı: {course_code} ({course_name}) [Kredi: {course_credits if course_credits is not None else '???'}]."
                )
            ...
            if backup_crn:
                try:
                    ...
                    backup_course_name, backup_course_credits = lesson_to_course[
                        backup_course_code
                    ]
...
def get_formatted_crn_list(crn_list: list[str]) -> list[str]:
    ...
    for crn_entry in crn_list:
        if ":" in crn_entry:
            ...
            formatted.append(
                f"{primary} ({lesson_name}) [Yedek: {backup} ({backup_name})]"
            )
        else:
...
if __name__ == "__main__":
    ...
    print(
        'Herhangi bir adımda "q" tuşuna basarak sihibarızı sonlandırabilirsiniz, son adıma kadar girdikleriniz kayıt edilmeyecektir..'
    )
    ...
    # Ask for the CRNs.
    print(
        "Almak istediğiniz derslerin CRN'lerini girin, bitirmek için hiç bir şey girmeden Enter tuşuna basın."
    )
    ...
    if crn_creds_range[0] != crn_creds_range[1]:
        print(
            f"Toplam kredi aralığı (yedek CRN'ler dahil): {crn_creds_range[0]} - {crn_creds_range[1]}"
        )
    ...
    # Ask for the SCRNs.
    wants_to_drop = (
        eval_input(input("Bırakmak istediğiniz ders(ler) var mı? [e/h]")).lower() == "e"
    )
    scrn_list = []
    if wants_to_drop:
        print(
    ...
    print("\n" * LINE_SPACES)
    print("Ders Seçim Zamanı: ", selection_datetime)
    print("\n" * LINE_SPACES)
    ...
    eval_input(
        input(
            'Eğer bir şeyler yanlış görünüyor ise "q" tuşuna basarak sihibazı sonlandırın, bilgileri kaydetmek için herhangi bir başka tuşa basın.'
        )
    )
    ...
    data_dict = {
        "account": {"username": user_name, "password": password},
        "time": {
        ...
        "courses": {"crn": crn_list, "scrn": scrn_list},
    }
